...
import info.papdt.blacklight.ui.common.CircularProgressView;
import pl.droidsonroids.gif.GifDrawable;
import pl.droidsonroids.gif.GifImageView;

import static info.papdt.blacklight.BuildConfig.DEBUG;

public class DirectMessageImageActivity extends AbsActivity /*implements OnPhotoTapListener*/
{
	private static final String TAG = DirectMessageImageActivity.class.getSimpleName();

	private ImageAdapter mAdapter;
	private ViewPager mPager;
	private DirectMessagesMediaApiCache mApiCache;
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		mApiCache = new DirectMessagesMediaApiCache(this);
	...
	public boolean onOptionsItemSelected(MenuItem item) {
		...
		if (id == android.R.id.home) {
			...
			if (!mLoaded[current]) {
				...
				String path = mApiCache.saveLargePic(mFid);
				if (id == R.id.save){
					if (path == null) {
	...
	private class ImageAdapter extends PagerAdapter {
		...
		public Object instantiateItem(ViewGroup container, int position) {
			...
			if (v != null) {
				...
				LinearLayout ll = new LinearLayout(DirectMessageImageActivity.this);
				ll.setGravity(Gravity.CENTER);
				CircularProgressView p = new CircularProgressView(DirectMessageImageActivity.this);
				int w = (int) Utility.dp2px(DirectMessageImageActivity.this, 50);
				ll.addView(p, w, w);
		...
		protected void onPostExecute(Object[] result) {
			...
			if (img != null) {
				...
				if (!img.endsWith(".gif")) {
					...
					final SubsamplingScaleImageView iv = new SubsamplingScaleImageView(DirectMessageImageActivity.this);
					...
					try {
						...
						GifImageView iv = new GifImageView(DirectMessageImageActivity.this);
